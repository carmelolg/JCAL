...
### I upgraded from 2.1.0 and my code no longer compiles — `setInitalState` not found.

The typo methods `setInitalState` and `getInitalState` were **removed** in 2.2.0.
Rename every call site to `setInitialState` / `getInitialState`. This is a simple
find-and-replace:

```bash
# Unix / macOS
grep -r "setInitalState\|getInitalState" src/
sed -i '' 's/setInitalState/setInitialState/g; s/getInitalState/getInitialState/g' <file>
```

See the [Changelog](../appendix/changelog/) for the full migration guide.
...
### Why is the builder method called `setInitialState`?

`setInitialState(List<Cell>)` is the standard, correctly-spelled API. In versions prior
to 2.2.0 the method was misspelled `setInitalState` (one `t`) — that typo has now been
removed. Always use `setInitialState`.
...
1. **Initial condition** — Did you call `setInitialState(...)` with a non-empty list?

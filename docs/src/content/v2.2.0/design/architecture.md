...
│   ├── AbstractCellularAutomataRule       ← shared listener management + buffer swap helper
│   ├── CellularAutomataRule               ← abstract; extend to define the transition rule
│   ├── CellularAutomataParallelRule       ← parallel variant; extend for multi-threaded rules
│   ├── CellularAutomataRunner             ← internal — Callable<Void> for parallel transition
│   ├── CellularAutomataRefinementRunner   ← internal — Callable<Void> for parallel refinement
│   └── GenerationListener                 ← @FunctionalInterface; notified after each generation
│
├── grid/
│   ├── Cell                               ← one cell; has a CellState + int[] coordinates
│   ├── CellState                          ← a cell's state; key (String) + arbitrary value Object
...
│   ├── NeighborhoodFactory                ← internal — resolves 2D/3D/4D class by type+dimension
...
│
├── utils/
│   └── Utils                              ← internal helpers (isInside, cloneGrid) — final, non-instantiable
│
└── examples/
...
| `AbstractCellularAutomataRule` | **Extension point** | Base class for all rule types; provides listener management |
| `CellularAutomataRule` | **Extension point** | Extends `AbstractCellularAutomataRule`; subclass to define sequential rule |
...
| `CellState` | **Public API** | Create instances for each state your CA needs; `getKey()` returns `String` |
| `NeighborhoodType` | **Public API** | Pass to builder for built-in neighbourhoods |
| `NeighborhoodFactory` | **Internal** | Do not use directly; resolves neighborhood by type and dimension |
...
| `CellularAutomataParallelRule` | **Extension point** | Extends `AbstractCellularAutomataRule`; subclass for parallel rule |
...
| `Utils` | Internal helper | `final` class — cannot be subclassed; use static methods only |

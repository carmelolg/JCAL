...
`setInitialState(List<Cell>)` is the standard, correctly-spelled API introduced
in 2.2.0. In prior versions the method was misspelled `setInitalState` (one `t`)
as a historical artifact. Always use `setInitialState`.
...
1. **Initial condition** — have you called `setInitialState(...)` with a non-empty list?
